...
export default function Signup() {
  ...
  const signInWithGoogle = async () => {
    signInWithPopup(auth, new GoogleAuthProvider())
      ...
      .catch((err) => alert('A login error occured'));
  };
  ...
  const signInWithPwd = async () => {
    if (pwd.current.value !== pwdRepeat.current.value) {
      setPwdMismatch(true);
      return;
    }

    createUserWithEmailAndPassword(auth, mail.current.value, pwd.current.value)
      .then((user) => {
        // Signed in

        updateProfile(auth.currentUser, {
          displayName: `${lastName.current.value} ${firstName.current.value}`,
        }).then(window.location.replace('/sets'));
      })
      .catch((error) => {
        ...
        if (errorCode === 'auth/invalid-email') setInvalidMail(true);
      });
  };
  return (
            ...
            <form action="#" className="mt-8 grid grid-cols-6 gap-6">
              ...
              <div className="col-span-6">
                ...
                <input
                  ...
                  name="email"
                  ref={mail}
                  className={`w-full mt-1 text-sm text-gray-700 bg-white ${
                    invalidMail ? 'border-red-400' : 'border-gray-200'
              ...
              <div className="col-span-6">
                ...
                <input
                  ...
                  name="pwd1"
                  className={`w-full mt-1 text-sm text-gray-700 bg-white ${
                    pwdMismatch ? 'border-red-400' : 'border-gray-200'
                  } rounded-md shadow-sm`}
                />
              </div>
              ...
              <div className="col-span-6">
                ...
                >
                  {pwdMismatch ? (
                    <p className="text-red-700">Repeat password</p>
                  ) : (
                    'Repeat password'
                  )}
                </label>
              ...
              <div className="col-span-6 sm:flex sm:items-center sm:gap-4">
                <button
                  ...
                  type="button"
                  className="inline-block px-12 py-3 text-sm font-medium text-white bg-blue-600 border border-blue-600 transition rounded-md shrink-0 hover:bg-transparent hover:text-blue-600 focus:outline-none focus:ring active:text-blue-500"
                >
                  Create account
                </button>
                ...
                <p className="mt-4 text-sm text-gray-500 sm:mt-0">
                  ...
                  <a
                    className="inline-block p-3 ml-3 hover:underline text-sm font-medium transition rounded-md shrink-0 hover:bg-transparent focus:ring "
                    href="/Login"
                  >
                    Login
                  </a>
